...
const calculateVoronoi = require("./helper-functions/calculateVoronoi");
const reGraph = require("./helper-functions/reGraph");
const reMarkFeatures = require("./helper-functions/reMarkFeatures");
const { getNameBases } = require("./helper-functions/names-generator");

const loadFromMapFile = ( docSet, docID = "example", mapUrl ) => {
    //Lots of the parsing code taken from save-and-load.js
    fs.readFile( __dirname + mapUrl, 'utf8', (err, data) => {
        if (err) {
            console.log("Cannot load map from URL: ", err);
            return;
        }
        else {
            return parseMapData(data, docSet, docID);
        }
    } )
};

const parseMapData = ( loadedMap, docSet, docId ) => {
    ...
    console.time("Automerge.from");
    console.log("Creating document from");
    const loadedDoc = Automerge.from( docData );
    console.timeEnd("Automerge.from");
    console.time("docset");
    docSet.setDoc(docId, loadedDoc);
